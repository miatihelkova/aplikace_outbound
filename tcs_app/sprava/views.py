...
from django.core.paginator import Paginator # <-- OPRAVA 3: Import Paginator
...
def upload_contacts(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if result["success"]:
                # --- OPRAVA 2 (i pro kontakty): Vytvoříme detailní zprávu ---
                success_message = (
...
def sprava_vratek(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # --- OPRAVA 1: Odebrána první zpráva, aby se neduplikovala ---
            # messages.info(request, "Soubor byl přijat...")
            
            summary = import_vratky_from_excel(excel_file)
            
            if summary["success"]:
                # --- OPRAVA 2: Vytvoříme jednu detailní zprávu se souhrnem ---
                success_message = (
            ...
            # Pokud formulář není validní, chyby se zobrazí automaticky
            pass
        
        return redirect('sprava:vratky')

    # --- OPRAVA 3: Přidáno stránkování pro zobrazení všech vratek ---
    form = VratkaForm()
    vratky_list = Vratka.objects.select_related('kontakt').order_by('-datum_importu')
    
    paginator = Paginator(vratky_list, 50)  # 50 vratek na stránku
    page_number = request.GET.get('page')
    vratky_page = paginator.get_page(page_number)

    context = {'form': form, 'vratky_page': vratky_page} # Změna 'vratky' na 'vratky_page'
    return render(request, 'sprava/sprava_vratek.html', context)

# ... zbytek souboru (contacts_list, kontakt_detail, správa uživatelů atd.) zůstává beze změny ...
@login_required
@user_passes_test(is_admin)
